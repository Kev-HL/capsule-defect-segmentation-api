"""
This module contains all functions used to create a TensorFlow model for defect
detection and localization.
The model uses transfer learning with a configurable backbone and custom heads for
classification and segmentation.
The model architecture supports different backbone networks and mask head types
(FCN-lite and U-Net-lite).
"""
...
def create_base_model(run_cfg, backbone_class_map) -> tf.keras.Model:
    ...
    Args:
        run_cfg (OmegaConf object): Contains the information of the global YAML config
        files plus those specific to the experiment begin run where the model parameters
        are defined.
        input_tensor (tf.Tensor): The input tensor for the model, which is created on
        the model_creation function.
        backbone_class_map (dict): A mapping of backbone class names to their
        corresponding TensorFlow Keras application classes, found on section 1.2
    Returns:
        base_model: A TensorFlow Keras model instance representing the base model
        without the top classification layers.
    """
    # Validate backbone configuration exists
    if run_cfg.base not in run_cfg.backbone.keys():
        raise ValueError(
            f"Unsupported backbone {run_cfg.base}. "
            f"Supported backbones are: {list(run_cfg.backbone.keys())}"
        )
    ...
    base_model = backbone_cls(
        include_top=False, weights="imagenet", pooling=None, **extra_args
    )
    # Assign a config based name to the base model so that it can be identified later
    # for freezing/unfreezing
    name = run_cfg.base
    ...
    # FCN-Lite style head
        ...
        return tf.keras.Model(
            inputs=base_model.input, outputs=base_model.output, name=name
        )
...
def sep_conv_bn_relu(x, filters, name, kernel_size=3, dropout_rate=0.0) -> tf.Tensor:
    ...
    x = tf.keras.layers.SeparableConv2D(
        filters, kernel_size, padding="same", use_bias=False, name=f"{name}_sepconv"
    )(x)
...
def mask_guided_pooling(inputs) -> tf.Tensor:
    ...
    pooled = tf.reduce_sum(weighted_features, axis=[1, 2]) / tf.squeeze(
        mask_sum, axis=[1, 2]
    )  # [B, C]
    return pooled
...
def build_mask_head_fcn_lite(feat, run_cfg) -> tf.Tensor:
    ...
    for i in range(blocks):
        x = sep_conv_bn_relu(
            x, filters, name=f"fcnlite_block{i+1}", dropout_rate=dropout
        )
...
def build_mask_head_unet_lite(skips, run_cfg) -> tf.Tensor:
    ...
    if len(feats) < 2:
        raise ValueError(
            "U-Net-lite requires at least two skip stages (e.g., C5 and C4)."
        )

    x = feats[0]  # start from deepest
    for i, skip in enumerate(feats[1:], start=1):
        x = tf.keras.layers.UpSampling2D(
            size=2, interpolation="bilinear", name=f"unet_up{i}"
        )(x)
        ...
        x = sep_conv_bn_relu(
            x,
            base_filters // (2 ** (i - 1)),
            name=f"unet_dec{i}_a",
            dropout_rate=dropout,
        )
        x = sep_conv_bn_relu(
            x,
            base_filters // (2 ** (i - 1)),
            name=f"unet_dec{i}_b",
            dropout_rate=dropout,
        )
...
def create_model(
    run_cfg, num_classes=6, backbone_class_map=backbone_class_map
) -> tf.keras.Model:
    """
    ...
    Args:
        ...
        backbone_class_map (dict): A mapping of backbone class names to their
        corresponding TensorFlow Keras application classes, found on section 1.2
        num_classes (int): The number of classes for the classification head output
        layer. Obtained dynamically in section 1.3 Metadata.
    Returns:
        model: A TensorFlow Keras model instance representing the complete model ready
        for training.
    """
    # Input layer
    target_size = tuple(run_cfg.input_size)
    input_image = tf.keras.Input(
        shape=(int(target_size[0]), int(target_size[1]), 3), name="image"
    ...
    # BN params (gamma/beta) to train unless you also set BN layers' trainable=False.
    ...
    if run_cfg.label_head.get("independent_FCN", False):
        ...
        for i in range(run_cfg.label_head.FCN_blocks):
            x = sep_conv_bn_relu(
                x,
                run_cfg.label_head.FCN_filters,
                name=f"class_fcnlite_block{i+1}",
                dropout_rate=run_cfg.label_head.dropout,
            )
        mask_for_cls = tf.keras.layers.Conv2D(
            1, 1, activation="sigmoid", name="classification_mask"
        )(x)

    else:
        mask_for_cls = tf.keras.layers.Lambda(stop_gradient_fn, name="stop_gradient")(
            output_mask
        )  # Stop gradient flow from classification head to mask head

    mask_down = tf.keras.layers.Resizing(
        feat_map.shape[1], feat_map.shape[2], interpolation="bilinear"
    )(
        mask_for_cls
    )  # Downsample mask to match feature map size
    mask_pooled = tf.keras.layers.Lambda(
        mask_guided_pooling,
        output_shape=mask_guided_pooling_output_shape,
        name="mask_guided_pooling",
    )(
        [feat_map, mask_down]
    )  # output_shape=(feat_map[0], feat_map[3])
    ...
    x_cls = tf.keras.layers.Dense(run_cfg.label_head.dense_units[0], activation="relu")(
        final_features
    )
    ...
    x_cls = tf.keras.layers.Dense(run_cfg.label_head.dense_units[1], activation="relu")(
        x_cls
    )
    x_cls = tf.keras.layers.BatchNormalization()(x_cls)
    if run_cfg.label_head.dropout and run_cfg.label_head.dropout > 0.0:
        x_cls = tf.keras.layers.Dropout(run_cfg.label_head.dropout)(x_cls)
    output_label = tf.keras.layers.Dense(
        num_classes, activation="softmax", name="label"
    )(x_cls)
    ...
    model.backbone = (
        base_model  # Attach handle as reference for freezing/unfreezing it later on
    )
