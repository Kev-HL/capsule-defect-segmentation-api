"""
This module contains all functions used to create and process TensorFlow datasets for
training, validation, and testing.
It includes functions for dataset creation from DataFrames, preprocessing (image
reading, resizing, mask handling),data augmentation (light and aggressive),
and final dataset assembly ready for model training and evaluation.
"""
...
def create_tf_dataset(df) -> tf.data.Dataset:
    """
    Creates a TensorFlow dataset from a DataFrame containing image paths, labels,
    and mask paths.
    Args:
        df (DataFrame): DataFrame containing 'image_path', 'label', and 'mask_path'
    Returns:
...
def create_preprocess_fn(
    ...
    [tf.Tensor, tf.Tensor, tf.Tensor], Tuple[tf.Tensor, tf.Tensor, tf.Tensor]
]:
    """
    Creates a preprocessing function for image data, so that it can be used in a
    TensorFlow dataset pipeline via '.map()'.

    Args:
        cfg (OmegaConf object): Contains the information of the YAML config file of the
        experiment (maybe merged with others), which contains input size to be used.
    Returns:
        preprocess_sample: A function that preprocesses an image sample and its mask if
        exists, if not creates an empty one.
    """
    ...
    def preprocess_sample(
        ...
        """
        Preprocess a single sample: read image, resize, handle mask presence, and return
        tensors.
        Args:
        ...
        Returns:
            ...
            mask (tf.Tensor): Preprocessed mask tensor of shape [H, W, 1], or empty mask
            if none exists.
        """
        ...
        image = tf.io.read_file(
            image_path
        )  # Read the image file as a binary string with TF
        image = tf.image.decode_png(
            image, channels=3
        )  # Decode the image to a tensor of shape [H, W, C]
        # with values as uint8 in range [0, 255]
        image = tf.cast(
            image, tf.float32
        )  # Change values from uint8 to float32 keeping scale [0, 255]
        ...
        has_mask = tf.logical_and(
            has_defect, has_path
        )  # Mask exists only if defect present and path is not empty
...
def light_augment_fn(image, label, mask, cfg) -> Tuple[tf.Tensor, tf.Tensor, tf.Tensor]:
    ...
    image = tf.image.random_contrast(
        image, lower=cfg.light.contrast_lower, upper=cfg.light.contrast_upper
    )
    ...
    image = tf.clip_by_value(
        image, 0.0, 1.0
    )  # Ensure pixel values remain in [0, 1], TF operations do not clip by default
    mask = tf.where(
        tf.math.greater(mask, 0.0), 1.0, 0.0
    )  # Binarize the mask again after augmentation

    # Scale back image to [0, 255] [backbone preprocessing layer expects this range]
    image = image * 255.0
...
def translate_image(image, dx, dy, fill_mode, interpolation) -> tf.Tensor:
    ...
    Args:
        ...
        fill_mode (str): Fill mode for areas outside the image ('CONSTANT', 'REFLECT'..)
...
def aggressive_augment_fn(
    image, label, mask, cfg
) -> Tuple[tf.Tensor, tf.Tensor, tf.Tensor]:
    """
    ...
    image = tf.image.random_brightness(
        image, max_delta=cfg.aggressive.brightness_max_delta
    )
    image = tf.image.random_contrast(
        image, lower=cfg.aggressive.contrast_lower, upper=cfg.aggressive.contrast_upper
    )
    image = tf.image.random_saturation(
        image,
        lower=cfg.aggressive.saturation_lower,
        upper=cfg.aggressive.saturation_upper,
    )
    ...
    dx = (
        tf.random.uniform(
            [],
            -cfg.aggressive.max_translation_shift,
            cfg.aggressive.max_translation_shift,
        )
        * cfg.input_size[1]
    )  # shift up to ±MAX% image width
    dy = (
        tf.random.uniform(
            [],
            -cfg.aggressive.max_translation_shift,
        ...
        * cfg.input_size[0]
    )  # shift up to ±MAX% image height
    dx = tf.cast(tf.round(dx), tf.float32)
    # Note: tf.math.round() rounds half to even by default
...
def create_augment_fn(
    cfg, light_augment_fn=light_augment_fn, aggressive_augment_fn=aggressive_augment_fn
) -> Callable[
    [tf.Tensor, tf.Tensor, tf.Tensor], Tuple[tf.Tensor, tf.Tensor, tf.Tensor]
]:
    """
    Creates an augmentation function that applies different augmentations based on the
    class label.
    Args:
        cfg (OmegaConf object): Contains the information of the YAML config file of the
        experiment.
    ...
    Returns:
        augment_sample: A function that applies the appropriate augmentation based on
        the label.
    """

    def augment_sample(image, label, mask) -> Tuple[tf.Tensor, tf.Tensor, tf.Tensor]:
        """
        Applies light augmentation for 'good' class (label == 0) and aggressive
        augmentation for 'defect' classes (label != 0).
        """
        return tf.cond(
            tf.equal(label, 0),
...
def augment_dataset(
    ds, cfg, light_aug_fn=light_augment_fn, aggressive_aug_fn=aggressive_augment_fn
) -> tf.data.Dataset:
    """
    Build augmented-only dataset via repeat+filter,
    then return base + augmented (concat).
    Assumes cfg has:
      - MULTIPLIER_GOOD (float), MULTIPLIER_DEFECT (float)
      - REPEAT_FACTOR (int)
    Args:
        ds (tf.data.Dataset): Input dataset to augment.
        cfg (OmegaConf object): Contains the information of the YAML config file.
    ...
    def should_keep(image, label, mask) -> tf.Tensor:
        """
        Determines whether to keep a sample based on its label and the configured keep
        probabilities.
        """
    ...
    aug_ds = (
        ds.repeat(int(cfg.REPEAT_FACTOR))
...
def format_for_model(image, label, mask) -> Tuple[tf.Tensor, Dict[str, tf.Tensor]]:
    ...
    From Tuple (image, label, mask) to Tuple (image, targets_dict)
    with targets_dict = {'label': label, 'mask': mask}.
    """
...
def train_dataset_assembly(
    train_df, val_df, cfg
) -> Tuple[tf.data.Dataset, tf.data.Dataset, int]:
    """
    Creates processed training and validation datasets ready for model training from
    input DataFrames.

    Args:
        train_df (DataFrame): DataFrame containing training data (split).
        val_df (DataFrame): DataFrame containing validation data (split).
        cfg (OmegaConf object): Contains the information of augment.yaml and the
        experiment specific YAML config file.
        All configuration settings for the data pipeline are included there.
    Returns:
        augmented_train_ds: A fully processed and prepared tf.data.Dataset object for
        training, with augmentation applied (includes original and augmented samples).
        val_ds: A fully processed and prepared tf.data.Dataset object for
        validation (no augmentation).
        approx_train_samples: An integer approximating the total number of the training
        samples after augmentation (approximately because of the probabilistic
        augmentation).
    """
    # Dataset creation
    ...
    approx_train_samples = int(
        good_count * (float(cfg.MULTIPLIER_GOOD) + 1.0)
        + defect_count * (float(cfg.MULTIPLIER_DEFECT) + 1.0)
    )

    # Data augmentation
    cfg.REPEAT_FACTOR = int(
        ceil(max(float(cfg.MULTIPLIER_GOOD), float(cfg.MULTIPLIER_DEFECT)))
    )  # Override repeat factor based on max multiplier to ensure enough samples
    augmented_train_ds = augment_dataset(preproc_train_ds, cfg)

    # Format for model
    formatted_augmented_train_ds = augmented_train_ds.map(
        format_for_model, num_parallel_calls=AUTOTUNE
    )
    formatted_val_ds = preproc_val_ds.map(format_for_model, num_parallel_calls=AUTOTUNE)

    # Shuffle, batch and prefetch
    BUFFER_SIZE = max(
        1024, int(1.5 * approx_train_samples)
    )  # 1.5 times the target dataset size to ensure buffer > training dataset size
    augmented_train_ds = (
        formatted_augmented_train_ds.shuffle(
            BUFFER_SIZE, seed=int(cfg.SEED), reshuffle_each_iteration=True
        )
        .repeat()
...
def test_dataset_assembly(test_df, cfg) -> tf.data.Dataset:
    ...
    # Format for model
    formatted_test_ds = preproc_test_ds.map(
        format_for_model, num_parallel_calls=AUTOTUNE
    )
