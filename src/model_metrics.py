"""
This module contains custom metrics for evaluating classification and segmentation
models.
Metrics included:
...
def mean_iou(
    y_true, y_pred, eps=1e-6, threshold=0.5, empty_mask_thresh=0.01
) -> tf.Tensor:
    """
    Compute the Mean Intersection over Union (IoU) metric over the binarized predicted
    mask.
    Args:
    ...
    union = (
        tf.reduce_sum(y_true, axis=axes) + tf.reduce_sum(y_pred_bin, axis=axes) - inter
    )
...
class MeanIoUDefectOnly(tf.keras.metrics.Metric):
    ...
    def __init__(
        self,
        mask_threshold=0.5,
        empty_mask_thresh=0.01,
        name="mean_iou_defect_only",
        **kwargs
    ) -> None:
        """
        ...
        Args:
            ...
            **kwargs: Additional keyword arguments for the base Metric class (passed
            to super()).
        """
        ...
        )  # Sum of IoUs for defect-only samples
        self.total = self.add_weight(
            name="total", initializer="zeros"
        )  # Count of defect-only samples
...
class MeanDiceDefectOnly(tf.keras.metrics.Metric):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.sum = self.add_weight(
            name="sum", initializer="zeros"
        )  # Sum of Dice scores for defect-only samples
        self.total = self.add_weight(
            name="total", initializer="zeros"
        )  # Count of defect-only samples
...
class DiceAtThreshold(tf.keras.metrics.Metric):
    ...
    def __init__(
        ...
        **kwargs
        ...
        )  # Count of samples passing the Dice threshold
        self.total = self.add_weight(
            name="total", initializer="zeros"
...
class DiceAtThresholdDefectOnly(tf.keras.metrics.Metric):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.count = self.add_weight(
            name="count", initializer="zeros"
        )  # Count of defect-only samples passing the Dice threshold
        self.total = self.add_weight(
            name="total", initializer="zeros"
        )  # Count of defect-only samples
    ...
    def update_state(self, y_true, y_pred, sample_weight=None) -> None:
        ...
        samples_defect_only_over_threshold = tf.logical_and(
            has_defect, has_dice_over_threshold
        )
        self.count.assign_add(
            tf.reduce_sum(tf.cast(samples_defect_only_over_threshold, tf.float32))
        )
...
class IoUAtThreshold(tf.keras.metrics.Metric):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.count = self.add_weight(
            name="count", initializer="zeros"
        )  # Count of samples passing the IoU threshold
        self.total = self.add_weight(
            name="total", initializer="zeros"
        )  # Total number of samples
...
class IoUAtThresholdDefectOnly(tf.keras.metrics.Metric):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.count = self.add_weight(
            name="count", initializer="zeros"
        )  # Count of defect-only samples passing the IoU threshold
        self.total = self.add_weight(
            name="total", initializer="zeros"
        )  # Count of defect-only samples
    ...
    def update_state(self, y_true, y_pred, sample_weight=None) -> None:
        ...
        samples_defect_only_over_threshold = tf.logical_and(
            has_defect, has_iou_over_threshold
        )
        self.count.assign_add(
            tf.reduce_sum(tf.cast(samples_defect_only_over_threshold, tf.float32))
        )
...
class defect_only_accuracy(tf.keras.metrics.Metric):
    ...
    def __init__(self, name="defect_only_accuracy", **kwargs) -> None:
        ...
        )  # Total correct predictions on defect samples
    ...
    def result(self) -> tf.Tensor:
        """
        Return the Defect-Only Accuracy metric value or NaN if no defect samples
        have been seen.
        """
