...
Usage (inside container):
  python /app/measure_inference.py \
    --image static/samples/sample_0.png \
    --model ./final_model.tflite \
    --num_threads 2 \
    --repeats 200 \
    --warmup 20

Note 1: script file not included in image, copy it manually if needed.
Note 2: psutil is commented out in the requirements.txt for building the image;
uncomment it if needed or install manually (tested with psutil==7.1.3).
"""
...
# Note: psutil is commented out in the requirements.txt for building the image;
# uncomment it if needed or install manually (tested with psutil==7.1.3).
try:
    import psutil
except ImportError:
...
def measure_inference(
    ...
    sampler = threading.Thread(
        ...
        args=(psutil.Process().pid, mem_sample_interval, stop_event, samples),
        daemon=True,
    )
    sampler.start()
    ...
    stats = {
        ...
        "p95_ms": (
            statistics.quantiles(timings, n=100)[94] if len(timings) >= 100 else None
        ),
        "p99_ms": (
            statistics.quantiles(timings, n=100)[98] if len(timings) >= 100 else None
        ),
...
def main() -> None:
    ...
    parser.add_argument(
        "--model",
        default="./final_model.tflite",
        help="Path to tflite model inside the container",
    )
    parser.add_argument(
        "--image",
        default="static/samples/sample_0.png",
        help="Path to a sample PNG inside the container",
    )
    parser.add_argument(
        "--num_threads",
        type=int,
    ...
    if psutil is None:
        raise SystemExit(
            "psutil is required for memory measurement; please install it."
        )
    ...
    if num_threads > 1:
        try:
            ...
            print(
                f"Warning: num_threads={num_threads} not supported, "
                f"falling back to single-threaded interpreter."
            )
