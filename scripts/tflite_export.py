...
Arguments provided via in-line command line:
    ...
    # --data_path: Path to the CSV file containing image paths for representative
                   dataset (only if used for full integer quantization)
...
def main() -> None:
    ...
    Arguments provided via in-line command line:
        ...
        --output_path: Path to save the .tflite model file
        # --data_path: Path to the CSV file containing image paths for representative
                       dataset (only if used for quantization)
    """
    # Argument parsing
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to save the .tflite model file",
    )
    # parser.add_argument(
    #   '--data_path', type=str, required=True,
    #   help='Path to the CSV file containing image paths for representative dataset'
    # )
    ...
    if MODEL_PATH.is_file():
        ...
        model = tf.keras.models.load_model(
            MODEL_PATH, custom_objects=custom_objects, compile=False, safe_mode=True
        )
    else:
        raise FileNotFoundError(
            f"Model checkpoint not found at {MODEL_PATH}. Cannot evaluate."
        )
    ...
    ###################################################################################
    # Commented out code for full integer quantization (uncomment random import
    # and data path argument if needed)
