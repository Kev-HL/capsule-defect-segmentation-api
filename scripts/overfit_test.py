"""
This script performs an overfit test on a small subset of the training data to ensure
that the model and training pipeline are functioning correctly.
It randomly selects a few samples per class from the training dataset and attempts
to overfit the model on this small dataset.
...
The script performs the following steps:
    ...
    2. Randomly selects a small number of samples per class to create a subset for the
    overfit test.
    3. Preprocesses the selected data without augmentations, and formats it for
    model input.
    4. Creates the model as specified in the configuration file, unfreezing the
    backbone for training.
    5. Compiles the model with appropriate loss functions and metrics.
    6. Trains the model on the small dataset, using the subset for both training
    and validation.
    7. Evaluates the model on the validation set and saves the evaluation results
    to a CSV file.
"""
...
SAMPLES_PER_CLASS = 4  # Number of samples per class for overfit test
# (4 samples per 6 classes, 24 samples total, 20 positives)
...
def main() -> None:
    ...
    try:
        ...
        parser.add_argument(
            ...
            type=str,
            required=True,
            help="Path to overfit test config file",
        )
        ...
        ov_df = raw_train_df.groupby("label").sample(
            n=SAMPLES_PER_CLASS, random_state=overf_cfg.SEED
        )
        ...
        train_ds = (
            formatted_ds.shuffle(
                len(ov_df), seed=overf_cfg.SEED, reshuffle_each_iteration=True
            )
            .batch(overf_cfg.BATCH_SIZE)
            .prefetch(AUTOTUNE)
        )
    ...
    try:
        ...
        # Unfreeze backbone for overfit test (BN layers will remain in inference mode,
        # moving stats frozen, gamma and beta trainable)
        ...
        ES = tf.keras.callbacks.EarlyStopping(
            monitor=overf_cfg.monitor_metric,
            mode=overf_cfg.monitor_mode,
            patience=overf_cfg.epochs,  # patience=epochs disables early stopping
            restore_best_weights=True,  # but it will still restore best weights
            verbose=overf_cfg.verbose,
        )
        ...
        bce_dice_loss = create_bce_dice_loss(
            alpha=overf_cfg.bce_dice_loss_alpha, beta=overf_cfg.bce_dice_loss_beta
        )
        losses = {
        ...
        model.compile(
            optimizer=tf.keras.optimizers.Adam(
                learning_rate=overf_cfg.lr, weight_decay=overf_cfg.weight_decay
            ),
            loss=losses,
    ...
    try:
        ...
        model.fit(
            ...
            steps_per_epoch=(len(ov_df) + overf_cfg.BATCH_SIZE - 1)
            // overf_cfg.BATCH_SIZE,
            callbacks=[RLR, ES],
            verbose=overf_cfg.verbose,
        )
