...
Note 2: requests is commented out in the requirements.txt for building the image;
uncomment it if needed or install manually (tested with requests==2.32.5).
"""
...
# Third-party imports
# Note: requests is commented out in the requirements.txt for building the image;
# uncomment it if needed or install manually (tested with requests==2.32.5).
try:
    import requests
except ImportError:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Predict endpoint URL inside container (use 127.0.0.1)",
    )
    parser.add_argument(
        "--image",
        default="static/samples/sample_0.png",
        help="Image path inside the container",
    )
