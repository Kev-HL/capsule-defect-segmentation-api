"""
This script processes the MVTec Capsule dataset to create a CSV file with image paths,
class labels, and mask paths.
...
A copy of the license can be found in the root folder of the dataset
(data/capsule/license.txt).
Citations:
...
]  # There is one folder per defect class
...
def main() -> None:
    ...
    class_names = ["good"] + sorted(
        [cls for cls in df["class_name"].unique() if cls != "good"]
    )  # Ensure 'good' (no defect) is first so it encodes to 0
    class_to_idx = {
        name: idx for idx, name in enumerate(class_names)
    }  # Encoding dict for class names to indices
    df["label"] = df["class_name"].map(
        class_to_idx
    )  # Add a 'label' column with the encoded class index.
