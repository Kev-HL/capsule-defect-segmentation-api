"""
This script creates/loads a Keras model and trains it for a specified stage
(1 == backbone frozen or 2 == completely trainable) of training.
...
Arguments provided via in-line command line:
    ...
    --raw_train_path: Path to the raw training data CSV file (all training data left
    after reserving test set).
    --cfg_path: Path to the training configuration file (Omegaconf YAML format).

The script performs the following steps:
    1. Loads the raw training data from the specified CSV file and splits it into
    ...
    4. Compiles the model with appropriate loss functions, metrics, and optimizer
    settings.
    5. Trains the model for the specified number of epochs.
    6. Saves the best model checkpoint, tensorboard logs, and training logs to the
    specified trial directory.
"""
...
# import os
# # Use asynchronous GPU memory allocator
...
from model_builder import (
    ...
    backbone_class_map,
    mask_guided_pooling,
    mask_guided_pooling_output_shape as m_g_p_o_shape,
    align_like_ref,
    ...
    match optimizer:
        ...
        case "AdamW":
            return tf.keras.optimizers.AdamW(
                learning_rate=lr, weight_decay=cfg.weight_decay, clipnorm=cfg.clipnorm
            )
        case _:
...
def main() -> None:
    ...
    try:
        ...
        parser.add_argument(
            "--stage", type=int, required=True, help="Training stage (1 or 2)"
        )
        parser.add_argument(
        ...
        if stage not in [1, 2]:
            raise ValueError(
                "stage must be 1 (backbone frozen) or 2 (completely trainable)."
            )
        ...
        if not tf.config.list_physical_devices("GPU"):
            ...
            #   'No GPU detected by TF, training will be done CPU-only.'
            # )
        ...
        # Prepare TF datasets
        train_ds, val_ds, approx_train_samples = train_dataset_assembly(
            train_df, val_df, run_cfg
        )
    ...
    try:
        ...
        if stage == 1:
            ...
            # Create the model
            model = create_model(
                run_cfg, num_classes=num_classes, backbone_class_map=backbone_class_map
            )
            # Freeze backbone
            ...
            if MC_path.is_file() and run_cfg.st1_epochs > 0:
                ...
                custom_objects = {
                    ...
                    "mask_guided_pooling_output_shape": m_g_p_o_shape,
                ...
                model.backbone = model.get_layer(
                    run_cfg.base
                )  # Re-attach backbone handle

                # After loading, check for existing log to continue training from last
                # epoch (not best epoch)
                ...
                if log_1_path.is_file():
                    with open(log_1_path, "r") as f:
                        ...
                        initial_epoch = (
                            log_stage1["epochs_ran"] + 1
                            if "epochs_ran" in log_stage1
                            else 0
                        )
                ...
                model = create_model(
                    ...
                    backbone_class_map=backbone_class_map,
                )
            # Unfreeze backbone
            model.backbone.trainable = True
            # Optionally freeze BN layers to avoid updating Gamma and Beta during
            # fine-tuning (moving stats always frozen, hardcoded in model creation)
        ...
        metrics = {
            ...
            "mask": [
                ...
                tf.keras.metrics.BinaryCrossentropy(
                    name="bce_metric", from_logits=False
                ),
                dice_loss,
            ],
        }
    ...
    try:
        ...
        history = model.fit(
            ...
            steps_per_epoch=(approx_train_samples + run_cfg.BATCH_SIZE - 1)
            // run_cfg.BATCH_SIZE,
        ...
        log = {
            ...
            "lr_at_best_stage": (
                float(LRLog.lrs_begin[best_epoch - 1])
                if (LRLog and best_epoch > 0)
                else None
            ),
            "lr_reductions_to_best_stage": (
                int(LRLog.reductions_cum[best_epoch - 1])
                if (LRLog and best_epoch > 0)
                else 0
            ),
