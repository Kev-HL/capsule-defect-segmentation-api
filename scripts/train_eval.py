...
It is intended to be called from a higher-level script or orchestrating function that
...
Arguments provided via in-line command line:
    ...
    --data_path: Path to the raw training data (all data left after splitting off test
    set) or test CSV file
    --eval_set: Mode of evaluation, 'val' for repeating same split with the data as
    training and evaluation in val set, 'test' to evaluate directly on all data
    provided (ie test set)
...
The script performs the following steps:
    ...
    5. Logs the evaluation results and training parameters to a JSON file in the trial
    directory.
"""
...
from model_builder import (
    mask_guided_pooling,
    mask_guided_pooling_output_shape,
    align_like_ref,
    align_like_ref_output_shape,
    stop_gradient_fn,
)
...
def export_predictions(model, ds, class_map, output_dir) -> None:
    """
    Export model predictions on dataset to disk, including per-sample metadata, Dice and
    IoU scores.
    Dice and IoU scores are set to 1.0 for 'good' samples without masks.
    Metadata is saved as a CSV, and predictions/GT images and masks are saved as a
    NPZ file.
    Args:
...
def main() -> None:
    ...
    try:
        ...
        parser.add_argument(
            "--trial_dir", type=str, required=True, help="Path to trial directory"
        )
        parser.add_argument(
            "--data_path",
            type=str,
            required=True,
            help="Path to raw training data / test data",
        )
        parser.add_argument(
            "--eval_set",
            type=str,
            required=True,
            help='"val" for training split eval or "test" for final eval',
        )
        parser.add_argument(
        ...
        bce_dice_loss = create_bce_dice_loss(
            alpha=run_cfg.bce_dice_loss_alpha, beta=run_cfg.bce_dice_loss_beta
        )
        custom_objects = {
        ...
        if MC_path.is_file():
            model = tf.keras.models.load_model(
                MC_path, custom_objects=custom_objects, compile=True, safe_mode=True
            )
        else:
            raise FileNotFoundError(
                f"Model checkpoint not found at {MC_path}. Cannot evaluate."
            )
        ...
        if eval_set == "val":
            ...
            if "train_index" not in run_cfg or "val_index" not in run_cfg:
                ...
                split_ratio = run_cfg.val_size / (
                    1.0 - run_cfg.test_size
                )  # Adjusted val size after test split
    ...
    try:
        ...
        run_params = OmegaConf.to_container(
            run_cfg, resolve=True
        )  # Convert omegaconf object to regular dict
        run_params["backbone"] = {
            run_cfg.base: run_params["backbone"][run_cfg.base]
        }  # Keep only the used backbone parameters in the file
        with open(
            (trial_dir / "run_params.json"), "w"
        ) as f:  # Save run parameters to a JSON file
        ...
        if log_1_path.is_file():
            with open(log_1_path, "r") as f:
                ...
                lr_reductions_to_best_stage1 = log_stage1.get(
                    "lr_reductions_to_best_stage", 0
                )
        else:
            epochs_ran_st1 = 0
            best_epoch_st1 = 0
        ...
        if log_2_path.is_file():
            with open(log_2_path, "r") as f:
                ...
                lr_reductions_to_best_stage2 = log_stage2.get(
                    "lr_reductions_to_best_stage", 0
                )
        ...
        eff_epochs_to_best = int(best_epoch_st1) + int(
            best_epoch_st2
        )  # Effective epochs to the final model
        ...
        results = {
            ...
            "label_defect_only_acc": float(
                val_eval.get("label_defect_only_accuracy", 0.0)
            ),
            "label_defect_presence_acc": float(
                val_eval.get("label_defect_presence_accuracy", 0.0)
            ),
            "mask_mean_dice_defect_only": float(
                val_eval.get("mask_mean_dice_defect_only", 0.0)
            ),
            "mask_mean_iou_defect_only": float(
                val_eval.get("mask_mean_iou_defect_only", 0.0)
            ),
            "mask_dice_at_0_7_defect_only": float(
                val_eval.get("mask_dice_at_0_7_defect_only", 0.0)
            ),
            "mask_iou_at_0_5_defect_only": float(
                val_eval.get("mask_iou_at_0_5_defect_only", 0.0)
            ),
        ...
        if eval_set == "val":
            ...
            idx_to_class = dict(
                eval_df[["label", "class_name"]].drop_duplicates().values
            )
